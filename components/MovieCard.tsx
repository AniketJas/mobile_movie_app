...
const MovieCard = ({ id, poster_path, title, release_date, vote_average, original_language }: Movie) => {
  return (
    <Link href={`/movie/${id}`} asChild>
      <TouchableOpacity className='w-[30%]'>
        ...
        <View className='flex-row items-center justify-between'>
          ...
          <Text className='text-xs text-light-300 font-medium uppercase'>{original_language}</Text>
        </View>
      </TouchableOpacity>
    </Link>
  )
